...
let videosPerPage = 8;
let currentPage = localStorage.getItem("currentPage") || 1; // Obtener la página actual almacenada en localStorage si está disponible
...
function createPageButton(page) {
    ...
    button.addEventListener("click", () => {
        currentPage = page;
        loadPage();
        window.scrollTo(0, 280); // Posicionar en la parte superior al hacer clic en cualquier botón de página
    });
    // Cambiar el color del botón dependiendo de la página en la que estás
    if (page == currentPage) {
        button.classList.add("current-page");
